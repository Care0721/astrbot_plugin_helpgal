...
class GalgamePlugin(Star):
    ...
    def _get_args(self, event: AstrMessageEvent, cmd: str) -> str:
        ...
        idx = msg.find(cmd)
        if idx != -1:
            return msg[idx + len(cmd):].strip()
        return ""

    def _stars(self, score: float) -> str:
        ratio  = score / self.rating_max_score
        filled = int(ratio * 5)
    ...
    async def _ai_chat(self, prompt: str, event: AstrMessageEvent):
        if not self._AI_KEY:
            yield event.plain_result(
                "❌ 未配置 AI 密钥，请在 WebUI 插件配置中填写 api_key。"
            )
            return
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self._AI_URL, json=payload, headers=headers,
                    ...
                    if resp.status == 200:
                        ...
                        yield event.plain_result(
                            "❌ AI 认证失败（401），请检查 WebUI 中填写的 api_key 是否正确。"
                        )
                    else:
                        body = await resp.text()
                        yield event.plain_result(f"❌ AI 调用失败（{resp.status}）：{body[:200]}")
        except Exception as e:
            yield event.plain_result(f"❌ AI 请求出错：{e}")
    ...
    async def gal_search(self, event: AstrMessageEvent):
        ...
        args = self._get_args(event, "gal se")
        if not args:
    ...
    async def gal_route(self, event: AstrMessageEvent):
        ...
        args = self._get_args(event, "gal route")
        if not args:
    ...
    async def gal_endings(self, event: AstrMessageEvent):
        ...
        args = self._get_args(event, "gal endings")
        if not args:
    ...
    async def gal_char(self, event: AstrMessageEvent):
        ...
        args = self._get_args(event, "gal char").split(None, 1)
        if len(args) < 2:
    ...
    async def gal_vn(self, event: AstrMessageEvent):
        ...
        name = self._get_args(event, "gal vn")
        if not name:
    ...
    async def gal_vnid(self, event: AstrMessageEvent):
        ...
        vid = self._get_args(event, "gal vnid")
    ...
    async def gal_add(self, event: AstrMessageEvent):
        ...
        uid  = self._uid(event)
        game = self._get_args(event, "gal add")
        if not game:
    ...
    async def gal_done(self, event: AstrMessageEvent):
        ...
        uid  = self._uid(event)
        game = self._get_args(event, "gal done")
        if not game:
            yield event.plain_result("❌ 请提供游戏名称")
            return
    ...
    async def gal_pause(self, event: AstrMessageEvent):
        ...
        uid  = self._uid(event)
        game = self._get_args(event, "gal pause")
        if not game:
            yield event.plain_result("❌ 请提供游戏名称")
            return
        self.progress_data.setdefault(uid, {}).setdefault(game, {"added_at": datetime.now().strftime("%Y-%m-%d")})
    ...
    async def gal_note(self, event: AstrMessageEvent):
        ...
        uid  = self._uid(event)
        args = self._get_args(event, "gal note").split(None, 1)
        if len(args) < 2:
    ...
    async def gal_notes(self, event: AstrMessageEvent):
        ...
        uid  = self._uid(event)
        game = self._get_args(event, "gal notes")
        if not game:
    ...
    async def gal_cg_init(self, event: AstrMessageEvent):
        ...
        uid  = self._uid(event)
        args = self._get_args(event, "gal cg init").split(None, 1)
        if len(args) < 2:
    ...
    async def gal_cg_add(self, event: AstrMessageEvent):
        ...
        uid  = self._uid(event)
        args = self._get_args(event, "gal cg add").split(None, 1)
        if len(args) < 2:
    ...
    async def gal_cg_set(self, event: AstrMessageEvent):
        ...
        uid  = self._uid(event)
        args = self._get_args(event, "gal cg set").split(None, 1)
        if len(args) < 2:
    ...
    async def gal_cg_show(self, event: AstrMessageEvent):
        ...
        uid  = self._uid(event)
        game = self._get_args(event, "gal cg show")
        if not game:
    ...
    async def gal_rate(self, event: AstrMessageEvent):
        uid  = self._uid(event)
        args = self._get_args(event, "gal rate").split(None, 2)
        if len(args) < 2:
            yield event.plain_result(
    ...
    async def gal_review(self, event: AstrMessageEvent):
        game  = self._get_args(event, "gal review")
        if not game:
    ...
    async def gal_wish(self, event: AstrMessageEvent):
        uid  = self._uid(event)
        game = self._get_args(event, "gal wish")
        if not game:
            yield event.plain_result("❌ 请提供游戏名称")
            return
    ...
    async def gal_unwish(self, event: AstrMessageEvent):
        uid      = self._uid(event)
        game     = self._get_args(event, "gal unwish")
        wishlist = self.wishlist_data.get(uid, [])
        if game in wishlist:
    ...
    async def gal_tag(self, event: AstrMessageEvent):
        tag = self._get_args(event, "gal tag")
        if not tag:
    ...
    async def gal_talk(self, event: AstrMessageEvent):
        content = self._get_args(event, "gal talk")
        if not content:
            yield event.plain_result(
                "💬 格式：/gal talk <内容>\n"
    ...
    async def gal_remind(self, event: AstrMessageEvent):
        args = self._get_args(event, "gal remind")
        if not args:
